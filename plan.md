...
Verified margins with the tuned constants (`brain.md` §5): stay ≈ 15× toward the marker, deviate ≈ 2.1× toward the anomaly — both survive ±0.15 confidence jitter.
...
1. ☐ Stub sim → real brain → decisions printing with closing arithmetic
...
| 17 | `interface-contract.md` §2 | Observation at 10 Hz forces a GPU readback stall 10×/s | **Frames at 5 Hz**, pose/budget JSON at 10 Hz. BRAIN decides at 1 Hz, so nothing is lost. |
