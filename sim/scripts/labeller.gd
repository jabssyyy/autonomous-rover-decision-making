...
const CLASSES := ["rock", "anomaly", "marker"]
...
func _render_set(count: int) -> void:
	...
	for i in count:
		...
		rover.heading_deg = rng.randf_range(0.0, 360.0)
...
func _write_dataset_yaml(dir: String) -> void:
	var f := FileAccess.open(dir.path_join("data.yaml"), FileAccess.WRITE)
	if f == null:
		return
	f.store_string("# generated by sim/scripts/labeller.gd -- offline training data only\n")
	f.store_string("path: %s\ntrain: images\nval: images\nnc: %d\nnames: %s\n"
		% [dir, CLASSES.size(), str(CLASSES)])
	f.close()
